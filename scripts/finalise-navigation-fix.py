...
value = routes.read_text()
value = value.replace(
    'route("deals", "routes/deals-safe.tsx")',
...
replace_once(
    ...
    '} from "~/lib/opportunity-access.server";\n'
    'import { isOpportunitySchemaUnavailable } from '
    '"~/lib/opportunity-schema.server";\n',
)
replace_once(
    deals,
  ...
  try {
    ...
    return {
      ...
      opportunities: projects.results.map((project) => ({
...
room = Path("app/routes/deal-room.tsx")
replace_once(
    room,
    '} from "~/lib/opportunity-access.server";\n',
    '} from "~/lib/opportunity-access.server";\n'
...
navigation_test = Path("tests/navigation-destinations.test.ts")
value = navigation_test.read_text()
value = value.replace(
    'route("deals", "routes/deals-safe.tsx")',
    'route("deals", "routes/deals.tsx")',
)
...
expected = '''    expect(routeSource).toContain('route("deals", "routes/deals.tsx")');
    expect(routeSource).toContain(
      'route("deals/:dealSlug", "routes/deal-room.tsx")',
    );
'''
replacement = expected + '''    expect(readFileSync("app/routes/deals.tsx", "utf8")).toContain(
...
'''
replace_once(navigation_test, expected, replacement)

for filename in [
    "app/routes/deals-safe.tsx",
    "app/routes/deal-room-safe.tsx",
    ".github/navigation-fix-trigger",
    ".github/workflows/diagnose-navigation-fix.yml",
    ".github/workflows/finalise-navigation-fix.yml",
    "scripts/finalise-navigation-fix.py",
]:
