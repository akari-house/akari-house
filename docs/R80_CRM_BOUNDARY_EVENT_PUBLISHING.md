...
That means the duplication was not only visual. The House could operate a second commercial/CRM data model.

## R80 containment decision

R80 removes the duplicate CRM routes from the AKARI House route registry and removes those modules from House admin navigation.
...
7. obtain an operator backup;
8. only then create a later cleanup migration for deprecated House CRM tables.
...
- see all Events in `submitted` state;
- preview the public Event detail as the host can see it;
- verify the meeting destination;
