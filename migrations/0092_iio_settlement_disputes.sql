CREATE TABLE IF NOT EXISTS campaign_settlements (
  ...
  settlement_type TEXT NOT NULL DEFAULT 'cash'
    CHECK (settlement_type IN ('cash', 'token', 'mixed', 'other')),
  currency TEXT NOT NULL,
  token_symbol TEXT,
  payment_status TEXT NOT NULL DEFAULT 'pending'
    CHECK (payment_status IN ('pending', 'approved', 'processing', 'paid', 'failed', 'cancelled')),
  payment_method TEXT,
...
CREATE INDEX IF NOT EXISTS idx_campaign_settlements_campaign
  ON campaign_settlements(campaign_id, payment_status);

CREATE TABLE IF NOT EXISTS campaign_settlement_adjustments (
  id TEXT PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS campaign_disputes (
  ...
  dispute_type TEXT NOT NULL
    CHECK (dispute_type IN ('missing_submission', 'rejected_submission', 'allocation', 'settlement', 'other')),
  description TEXT NOT NULL,
  evidence_url TEXT,
  status TEXT NOT NULL DEFAULT 'open'
    CHECK (status IN ('open', 'reviewing', 'resolved', 'declined', 'withdrawn')),
...
CREATE INDEX IF NOT EXISTS idx_campaign_disputes_campaign
  ON campaign_disputes(campaign_id, status, created_at DESC);

CREATE TABLE IF NOT EXISTS campaign_final_reports (
  id TEXT PRIMARY KEY,
