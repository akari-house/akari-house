...
export const meta: Route.MetaFunction = () => [
  ...
  {
    name: "description",
    content: "Internal relationship ownership, follow-up and network intelligence.",
  },
];
...
export async function action({ request, context }: Route.ActionArgs) {
  ...
  if (displayName.length > 160 || companyName.length > 160 || source.length > 300)
  ...
  if (subjectUserId) {
    const member = await db
      ...
      .bind(subjectUserId)
      .first<{ id: string; email: string; displayName: string; roles: string }>();
  ...
  return (
    <div className="dashboard-shell">
      ...
      <main id="main-content" className="admin-main">
        ...
        {actionData?.error && (
          <p className="notice error" role="alert">{actionData.error}</p>
        )}
        {actionData?.saved && (
          <p className="notice success" role="status">Relationship saved.</p>
        )}

        <section className="admin-summary-grid" aria-label="Relationship summary">
          <article className="status-card"><span className="chapter">Records</span><h2>{loaderData.summary.total}</h2></article>
          <article className="status-card"><span className="chapter">Needs attention</span><h2>{loaderData.summary.attention}</h2></article>
          <article className="status-card"><span className="chapter">Strong / trusted</span><h2>{loaderData.summary.strong}</h2></article>
          <article className="status-card"><span className="chapter">Opted out</span><h2>{loaderData.summary.optedOut}</h2></article>
        </section>
              ...
              <select name="subjectUserId" defaultValue="">
                ...
                {loaderData.people.map((person) => (
                  <option key={person.id} value={person.id}>
                    {person.displayName} (@{person.username}){person.roles ? ` · ${person.roles}` : ""}
                  </option>
                ))}
              </select>
            </label>
            <label className="field"><span>External / fallback name</span><input name="displayName" maxLength={160} /></label>
            <label className="field"><span>Email</span><input name="email" type="email" maxLength={254} /></label>
            <label className="field"><span>Company</span><input name="companyName" maxLength={160} /></label>
            <label className="field">
              <span>Relationship type</span>
              <select name="relationshipType" defaultValue="other">
                {relationshipTypes.map((type) => <option key={type} value={type}>{relationshipTypeLabels[type]}</option>)}
              </select>
            </label>
            <label className="field">
              <span>AKARI owner</span>
              <select name="ownerUserId" required defaultValue={loaderData.user.id}>
                {loaderData.owners.map((owner) => <option key={owner.id} value={owner.id}>{owner.displayName} (@{owner.username})</option>)}
              </select>
            </label>
            <label className="field">
              <span>Strength</span>
              <select name="strength" defaultValue="known">
                {relationshipStrengths.map((strength) => <option key={strength} value={strength}>{relationshipStrengthLabels[strength]}</option>)}
              </select>
            </label>
            <label className="field">
              <span>Consent</span>
              <select name="consentStatus" defaultValue="unknown">
                {consentStatuses.map((status) => <option key={status} value={status}>{status.replace("_", " ")}</option>)}
              </select>
            </label>
            <label className="field"><span>Source / how we met</span><input name="source" maxLength={300} placeholder="Founder Space, referral, event..." /></label>
            <label className="field">
              <span>Related Project</span>
              <select name="projectId" defaultValue="">
                <option value="">No Project</option>
                {loaderData.projects.map((project) => <option key={project.id} value={project.id}>{project.title}</option>)}
              </select>
            </label>
            <label className="field"><span>Next action date</span><input name="nextActionAt" type="date" /></label>
            <label className="field"><span>Next action</span><input name="nextAction" maxLength={500} /></label>
            <label className="field field-full"><span>Internal note</span><textarea name="internalNote" rows={3} maxLength={3000} /></label>
            <div className="application-actions"><button className="button button-primary" type="submit" disabled={pending}>Create relationship</button></div>
          </Form>
        </section>
        ...
        <section className="status-card">
          ...
          <Form method="get" className="form-grid">
            <label className="field"><span>Search</span><input name="q" defaultValue={loaderData.query} /></label>
            <label className="field"><span>Type</span><select name="type" defaultValue={loaderData.typeFilter}><option value="">All types</option>{relationshipTypes.map((type) => <option key={type} value={type}>{relationshipTypeLabels[type]}</option>)}</select></label>
            <label className="field"><span>Status</span><select name="status" defaultValue={loaderData.statusFilter}><option value="">All statuses</option>{relationshipStatuses.map((status) => <option key={status} value={status}>{status}</option>)}</select></label>
            <div className="application-actions"><button className="button button-quiet" type="submit">Apply filters</button>{searchParams.toString() && <Link className="button button-quiet" to="/admin/relationships">Clear</Link>}</div>
          </Form>
        </section>
            ...
            <article className="application-card" key={record.id}>
              <div>
                <span className="chapter">
                  {relationshipTypeLabels[record.relationshipType]} · {relationshipStrengthLabels[record.strength]} · {record.status}
                  {record.needsAttention ? " · attention" : ""}
                </span>
                <h2>{record.name}</h2>
                <p>{record.companyName || record.projectTitle || "No company / Project linked"}</p>
                <p>Owner: {record.ownerName} · Last interaction: {dateLabel(record.lastInteractionAt)} · Interactions: {record.interactionCount}</p>
                <p>Next: {record.nextAction || "No action recorded"}{record.nextActionAt ? ` · ${dateLabel(record.nextActionAt)}` : ""}</p>
                <small>Consent: {record.consentStatus.replace("_", " ")}{record.source ? ` · Source: ${record.source}` : ""}</small>
              </div>
              <div className="application-actions">
                <Link className="button button-primary" to={`/admin/relationships/${record.id}`}>Open 360° view</Link>
                {record.memberUsername && <Link className="button button-quiet" to={`/profiles/${record.memberUsername}`}>Member profile</Link>}
                {record.projectSlug && <Link className="button button-quiet" to={`/projects/${record.projectSlug}`}>Project</Link>}
              </div>
            </article>
          ))}
          {!loaderData.relationships.length && <p>No relationship records match these filters.</p>}
        </div>
      </main>
    </div>
  );
