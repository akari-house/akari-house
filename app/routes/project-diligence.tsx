...
export async function loader({ request, context, params }: Route.LoaderArgs) {
  ...
  const project = await db.prepare(
    ...
    id: string; slug: string; title: string; founderUserId: string; dataRoomUrl: string;
  }>();
  ...
  const documents = await db.prepare(
    ...
    id: string; title: string; contentType: string; byteSize: number; createdAt: string;
  }>();
  ...
  if (isFounder) {
    const [investors, grants, requests, logs] = await Promise.all([
      ...
      db.prepare(
        ...
        id: string; documentId: string; documentTitle: string; investorUserId: string;
        investorName: string; username: string; canDownload: number; expiresAt: string; revokedAt: string | null;
      }>(),
      db.prepare(
        ...
        id: string; reason: string; status: string; expiresAt: string | null;
        createdAt: string; username: string; investorName: string;
      }>(),
      db.prepare(
        ...
        action: string; createdAt: string; documentTitle: string; memberName: string; username: string;
      }>(),
    ]);
    return {
      user, project, isFounder, documents: documents.results,
      investors: investors.results, grants: grants.results,
      requests: requests.results, logs: logs.results,
      ownRequest: null,
    };
  }
  ...
  const [grants, ownRequest] = await Promise.all([
    db.prepare(
      ...
      id: string; documentId: string; documentTitle: string; canDownload: number; expiresAt: string;
    }>(),
    db.prepare(
      ...
      id: string; reason: string; status: string; expiresAt: string | null; createdAt: string;
    }>(),
  ]);
  return {
    user, project, isFounder, documents: [], investors: [], requests: [], logs: [],
    grants: grants.results, ownRequest,
  };
}
...
export async function action({ request, context, params }: Route.ActionArgs) {
  ...
  if (intent === "request-data-room") {
    ...
    await db.batch([
      ...
      ).bind(crypto.randomUUID(), project.founderUserId,
  ...
  if (intent === "grant-document") {
    ...
    if (!documentId || !investorUserId || ![7, 14, 30, 60, 90].includes(days))
      return { error: "Choose a document, verified Investor and access period." };
    ...
    ).bind(investorUserId, documentId, project.id).first();
    ...
    await db.batch([
      ...
      db.prepare(
        `INSERT INTO document_access_grants
         ...
         VALUES (?, ?, ?, ?, ?, ?, datetime('now', ?))`,
      ).bind(crypto.randomUUID(), project.id, documentId, investorUserId, user.id,
        canDownload, `+${days} days`),
      db.prepare(
        `INSERT INTO notifications (id, user_id, kind, title, body, action_url)
         VALUES (?, ?, 'diligence.granted', 'Project document access granted', ?, ?)`,
      ).bind(crypto.randomUUID(), investorUserId,
      ...
      db.prepare(
        ...
        JSON.stringify({ documentId, investorUserId, days, canDownload: Boolean(canDownload) })),
    ]);
  ...
  if (intent === "revoke-grant") {
    ...
    await db.batch([
      ...
      ).bind(crypto.randomUUID(), user.id, project.id, JSON.stringify({ grantId })),
    ]);
  ...
  if (intent === "approve-data-room" || intent === "decline-data-room") {
    ...
    if (note.length < 5 || note.length > 500 || ![7, 14, 30, 60, 90].includes(days))
    ...
    ).bind(requestId, project.id).first<{ investorUserId: string }>();
    ...
    await db.batch([
      ...
      ).bind(approved ? "approved" : "declined", user.id, note, approved ? 1 : 0,
        `+${days} days`, requestId),
      db.prepare(
        `INSERT INTO notifications (id, user_id, kind, title, body, action_url)
         VALUES (?, ?, 'diligence.data_room_decision', ?, ?, ?)`,
      ).bind(crypto.randomUUID(), target.investorUserId,
...
export default function ProjectDiligence({ loaderData, actionData }: Route.ComponentProps) {
      ...
      <main id="main-content" className="admin-main">
        <header className="admin-heading">
          <div>
            ...
            <p>Permission-based project documents, data-room requests and access history.</p>
          </div>
        ...
        {loaderData.isFounder ? (
          <>
            <section className="admin-panel">
              ...
              {!loaderData.documents.length ? <p>Upload documents in the project editor first.</p> : (
                <Form method="post" className="form-stack">
                  <label>Document<select name="documentId" required>
                    <option value="">Choose a document</option>
                    {loaderData.documents.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}
                  </select></label>
                  <label>Verified Investor<select name="investorUserId" required>
                    <option value="">Choose an Investor</option>
                    {loaderData.investors.map((item) => <option key={item.id} value={item.id}>{item.displayName} (@{item.username})</option>)}
                  </select></label>
                  <label>Access period<select name="days" defaultValue="30">
                    {[7,14,30,60,90].map((days) => <option key={days} value={days}>{days} days</option>)}
                  </select></label>
                  <label className="inline-choice"><input type="checkbox" name="canDownload" value="yes" defaultChecked />Allow download</label>
                  <button className="button button-primary" name="intent" value="grant-document" disabled={pending}>Grant access</button>
                </Form>
              )}
            </section>
              ...
              <div className="application-list">
                {loaderData.grants.map((grant) => (
                  <article className="application-card" key={grant.id}>
                    <div><span className="chapter">{grant.revokedAt ? "revoked" : "active"}</span>
                      <h3>{grant.documentTitle}</h3><p>{grant.investorName} (@{grant.username})</p>
                      <small>Expires {new Date(grant.expiresAt).toLocaleString()}</small></div>
                    {!grant.revokedAt && <Form method="post"><input type="hidden" name="grantId" value={grant.id} />
                      <button className="button button-quiet" name="intent" value="revoke-grant" disabled={pending}>Revoke</button></Form>}
                  </article>
                ))}
              ...
              <div className="application-list">
                {loaderData.requests.map((item) => (
                  <article className="application-card" key={item.id}>
                    <div><span className="chapter">{item.status}</span><h3>{item.investorName} (@{item.username})</h3><p>{item.reason}</p></div>
                    {item.status === "pending" && <Form method="post" className="application-actions">
                      <input type="hidden" name="requestId" value={item.id} />
                      <label>Decision note<textarea name="decisionNote" minLength={5} maxLength={500} required /></label>
                      <label>Access period<select name="days" defaultValue="30">{[7,14,30,60,90].map((days) => <option key={days} value={days}>{days} days</option>)}</select></label>
                      <button className="button button-primary" name="intent" value="approve-data-room" disabled={pending}>Approve</button>
                      <button className="button button-quiet" name="intent" value="decline-data-room" disabled={pending}>Decline</button>
                    </Form>}
                  </article>
                ))}
                {!loaderData.requests.length && <p>No data-room requests yet.</p>}
              </div>
            </section>

            <section><h2>Recent document access</h2><div className="application-list">
              {loaderData.logs.map((log, index) => <article className="application-card" key={`${log.createdAt}:${index}`}><div><span className="chapter">{log.action}</span><h3>{log.documentTitle}</h3><p>{log.memberName} (@{log.username})</p></div><time>{new Date(log.createdAt).toLocaleString()}</time></article>)}
              {!loaderData.logs.length && <p>No document access has been recorded.</p>}
            </div></section>
          </>
        ) : (
          <>
            <section className="admin-panel"><h2>Your granted documents</h2>
              <div className="application-list">
                {loaderData.grants.map((grant) => <article className="application-card" key={grant.id}><div><h3>{grant.documentTitle}</h3><small>Access expires {new Date(grant.expiresAt).toLocaleString()}</small></div><a className="button button-primary" href={`/projects/${loaderData.project.slug}/documents/${grant.documentId}`}>{grant.canDownload ? "Download" : "Open"}</a></article>)}
                {!loaderData.grants.length && <p>No active document grants.</p>}
              </div>
            </section>
            <section className="admin-panel"><h2>Request external data-room access</h2>
              {loaderData.ownRequest && ["pending","approved"].includes(loaderData.ownRequest.status) ? <p>Your latest request is <strong>{loaderData.ownRequest.status}</strong>{loaderData.ownRequest.expiresAt ? ` until ${new Date(loaderData.ownRequest.expiresAt).toLocaleString()}` : ""}.</p> : <Form method="post" className="form-stack"><label>Why do you need access?<textarea name="reason" minLength={20} maxLength={800} required /></label><button className="button button-primary" name="intent" value="request-data-room" disabled={pending}>Request data-room access</button></Form>}
              {loaderData.ownRequest?.status === "approved" && loaderData.project.dataRoomUrl && <a className="button button-primary" href={loaderData.project.dataRoomUrl} target="_blank" rel="noreferrer">Open approved data room</a>}
            </section>
          </>
        )}
      </main>
