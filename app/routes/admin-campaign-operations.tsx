...
export async function action({ request, context }: Route.ActionArgs) {
  ...
  const internalNotes =
    formText(form.get("internalNotes")).slice(0, 4000) || null;
  if (
    !campaignId ||
    !["normal", "attention", "escalated"].includes(escalationStatus)
  ) {
...
export default function AdminCampaignOperations({
  ...
  const adminName = new Map(
    loaderData.admins.map((item) => [item.id, item.displayName]),
  );
  return (
      ...
      <main id="main-content" className="admin-main">
        <header className="admin-heading">
          <div>
            ...
            <p>
              Assign accountable moderators and preserve every operational
              change.
            </p>
          </div>
        ...
        <section className="admin-panel">
          ...
          <div className="member-home-stats">
            {loaderData.workload.map((item) => (
              <article key={item.id}>
                <strong>
                  {item.primaryCount} / {item.backupCount}
                </strong>
            ...
            <article className="application-card" key={campaign.id}>
              <div>
                <span className="chapter">
                  {campaign.status} · {campaign.escalationStatus ?? "normal"}
                </span>
                <h2>{campaign.title}</h2>
                <p>
                  Primary:{" "}
                  {campaign.primaryModeratorId
                    ? adminName.get(campaign.primaryModeratorId)
                    : "Unassigned"}
                </p>
                <p>
                  Backup:{" "}
                  {campaign.backupModeratorId
              ...
              <Form method="post" className="form-stack">
                ...
                <label>
                  Primary moderator
                  <select
                    name="primaryModeratorId"
                    defaultValue={campaign.primaryModeratorId ?? ""}
                  >
                    <option value="">Unassigned</option>
                    {loaderData.admins.map((item) => (
                ...
                <label>
                  Backup moderator
                  <select
                    name="backupModeratorId"
                    defaultValue={campaign.backupModeratorId ?? ""}
                  >
                    <option value="">Unassigned</option>
                    {loaderData.admins.map((item) => (
                      <option key={item.id} value={item.id}>
                ...
                <label>
                  ...
                  <select
                    ...
                    defaultValue={campaign.escalationStatus ?? "normal"}
                  >
                ...
                <label>
                  ...
                  <textarea
                    name="internalNotes"
                    maxLength={4000}
                    defaultValue={campaign.internalNotes ?? ""}
                  />
                </label>
