...
export const meta: Route.MetaFunction = () => [
  { title: "Relationship 360° | AKARI House" },
  {
    name: "description",
    content:
      "Internal relationship timeline, warm paths and connected AKARI activity.",
  },
];
...
export async function loader({ request, context, params }: Route.LoaderArgs) {
  ...
  const [interactionResult, ownerResult, peopleResult, projectResult] =
    await Promise.all([
      db
        .prepare(
  ...
  if (relationship.subjectUserId) {
    const [warmResult, projectRelationshipResult] = await Promise.all([
      ...
      db
        ...
        .bind(relationship.subjectUserId)
        .all<ProjectRelationshipRow>(),
    ]);
    warmPaths = prioritizeWarmPaths(
      warmResult.results.map((candidate) => ({
        ...candidate,
...
export async function action({ request, context, params }: Route.ActionArgs) {
  ...
  if (intent === "add-interaction") {
    ...
    const projectId =
      formText(form.get("interactionProjectId")).trim() || existing.projectId;
    if (!isInteractionType(interactionType))
      return { error: "Choose a valid interaction type." };
    if (summary.length < 2 || summary.length > 2000)
      return {
        error: "Interaction summary must be between 2 and 2,000 characters.",
      };
    if (occurredDate === undefined)
      return { error: "Use a valid interaction date." };
  ...
  const introducedByUserId =
    formText(form.get("introducedByUserId")).trim() || null;
  ...
  if (!isRelationshipStatus(status))
    return { error: "Choose a valid relationship status." };
  ...
  if (!existing.subjectUserId && !displayName && !email)
    return { error: "External relationships need a name or email." };
  if (
  ...
  if (
    nextAction.length > 500 ||
    conflictNote.length > 2000 ||
  ...
  if (introducedByUserId) {
    const introducer = await db
      ...
      .bind(introducedByUserId)
      .first<{ id: string }>();
    if (!introducer)
      return { error: "The selected introducer is no longer active." };
  }
  if (projectId) {
    const project = await db
      .prepare("SELECT id FROM projects WHERE id = ?")
      ...
      <main id="main-content" className="admin-main">
        ...
        <header className="admin-heading">
          <div>
            ...
            <p>
              ...
              {relationshipStrengthLabels[record.strength]} · Owner{" "}
              {record.ownerName}
              {record.needsAttention ? " · Needs attention" : ""}
            </p>
          </div>
        ...
        >
          <article className="status-card">
            <span className="chapter">Owner</span>
            <h2>{record.ownerName}</h2>
            <p>@{record.ownerUsername}</p>
          </article>
          <article className="status-card">
            <span className="chapter">Last interaction</span>
            <h2>{dateTimeLabel(record.lastInteractionAt)}</h2>
          </article>
          <article className="status-card">
            <span className="chapter">Next action</span>
            <h2>{record.nextAction || "Not set"}</h2>
            <p>{dateTimeLabel(record.nextActionAt)}</p>
          </article>
          <article className="status-card">
            <span className="chapter">Consent</span>
            <h2>{record.consentStatus.replace("_", " ")}</h2>
          </article>
        </section>

        <section className="status-card">
          <span className="chapter">Connected AKARI activity</span>
          <h2>360° account context</h2>
          <p>
            Opportunity: {loaderData.related.opportunityStatus ?? "none"} ·
            Campaigns: {loaderData.related.campaigns} · Agreements:{" "}
            {loaderData.related.agreements} · Introductions:{" "}
            {loaderData.related.introductions} · Active Data Room grants:{" "}
            {loaderData.related.activeDataRoomGrants}
          </p>
          {loaderData.projectRelationships.length > 0 && (
            <div className="application-list">
              {loaderData.projectRelationships.map((item) => (
                <article className="application-card" key={item.projectId}>
                  <div>
                    <span className="chapter">
                      {item.relationshipType} · {item.claimStatus}
                    </span>
                    <h3>{item.title}</h3>
                  </div>
                  <Link
        ...
        <section className="status-card">
          <span className="chapter">Warm path finder</span>
          <h2>Recorded paths, never guessed</h2>
          <p>
            Paths come only from accepted AKARI member connections and the
            recorded introducer. They are operational context, not an automated
            relationship score.
          </p>
          {record.introducedByName && (
            <p>
              <strong>Recorded introducer:</strong> {record.introducedByName}
              {record.introducedByUsername
                ? ` (@${record.introducedByUsername})`
                : ""}
            </p>
          )}
          {loaderData.warmPaths.length > 0 ? (
            <div className="application-list">
              {loaderData.warmPaths.map((path) => (
                <article className="application-card" key={path.userId}>
                  <div>
                    <span className="chapter">
                      {path.isOwner
                        ? "Relationship owner · direct connection"
                        : "Direct accepted connection"}
                    </span>
                    <h3>{path.displayName}</h3>
                    <p>
                      @{path.username} → {record.name}
                    </p>
                  </div>
                  <Link
                    className="button button-quiet"
                    to={`/profiles/${path.username}`}
                  >
                    Profile
                  </Link>
                </article>
            ...
            <p>
              No direct accepted AKARI-admin connection is recorded for this
              person.
            </p>
          )}
        </section>
        ...
        <section className="status-card">
          <span className="chapter">Relationship record</span>
          <h2>Ownership, source and follow-up</h2>
          <Form method="post" className="form-grid" aria-busy={pending}>
            <input type="hidden" name="intent" value="update" />
            <label className="field">
              <span>Fallback / external name</span>
              <input
                name="displayName"
            ...
            <label className="field">
              ...
              <select
                ...
                defaultValue={record.relationshipType}
              >
            ...
            <label className="field">
              <span>Strength</span>
              <select name="strength" defaultValue={record.strength}>
            ...
            <label className="field">
              <span>Consent</span>
              <select name="consentStatus" defaultValue={record.consentStatus}>
            ...
            <label className="field">
              ...
              <select
                name="introducedByUserId"
                defaultValue={record.introducedByUserId ?? ""}
              >
            ...
            <label className="field">
              ...
              <input
                type="date"
                name="nextActionAt"
                defaultValue={dateInputValue(record.nextActionAt)}
              />
            </label>
            ...
            <label className="field field-full">
              <span>Conflict / coordination note</span>
              <textarea
                name="conflictNote"
                defaultValue={record.conflictNote}
                rows={3}
                maxLength={2000}
              />
            </label>
        ...
        <section className="status-card">
          <span className="chapter">Interaction timeline</span>
          ...
          <Form method="post" className="form-grid" aria-busy={pending}>
            ...
            <label className="field field-full">
              ...
              <textarea
                ...
                maxLength={2000}
                rows={3}
                placeholder="Meeting outcome, introduction, Telegram follow-up, Space attendance..."
              />
            </label>
            <div className="application-actions">
              <button
          ...
          <div className="application-list">
            {loaderData.interactions.map((interaction) => (
              <article className="application-card" key={interaction.id}>
                <div>
                  <span className="chapter">
                    {interaction.interactionType} ·{" "}
                    {dateTimeLabel(interaction.occurredAt)}
                  </span>
                  <h3>{interaction.summary}</h3>
                  <p>
                    Recorded by {interaction.createdByName} (@
                    {interaction.createdByUsername})
                    {interaction.projectTitle
                      ? ` · ${interaction.projectTitle}`
                      : ""}
                  </p>
                </div>
            ...
            {!loaderData.interactions.length && (
