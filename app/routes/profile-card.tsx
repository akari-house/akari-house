...
const palettes = {
  sakura: { label: "Sakura", background: "#f04f87", ink: "#fff9f5", accent: "#ffd166" },
  midnight: { label: "Midnight", background: "#0b0d16", ink: "#fff9f5", accent: "#f04f87" },
  lantern: { label: "Lantern", background: "#ffd166", ink: "#17101a", accent: "#f04f87" },
} as const;

const platforms = ["x", "linkedin", "tiktok", "instagram", "facebook", "youtube"] as const;
...
export async function loader({ request, context }: Route.LoaderArgs) {
  ...
  const [profile, roles, socials, settings, opportunityStats, reputation, population] =
    await Promise.all([
      db
        .prepare(
        ...
        .bind(user.id)
        .first<{ displayName: string; headline: string; location: string; avatarKey: string }>(),
      db
      ...
      safeFirst(
        () =>
          db
            .prepare(
              `SELECT
                 (SELECT COUNT(*) FROM projects WHERE owner_user_id = ?) AS created,
                 (SELECT COUNT(*) FROM opportunity_access_requests WHERE investor_user_id = ?
                   AND status = 'approved') AS received`,
            )
...
function drawCard(
  ...
  ctx.fillText(settings.design === "passport" ? "MEMBER PASSPORT" : "MEMBER SIGNATURE", 86, 154);
  ctx.fillStyle = palette.ink;
  ...
  ctx.fillText(data.roles.map((role) => role[0].toUpperCase() + role.slice(1)).join(" · "), 90, portrait ? 470 : 548);
  ...
  ctx.fillStyle = palette.ink;
  const location = settings.showLocation && settings.countryCode
  ...
  if (languages.length) ctx.fillText("Languages · " + languages.join(" · "), 90, portrait ? 890 : 930);
  ctx.textAlign = "right";
...
export default function ProfileCard({ loaderData, actionData }: Route.ComponentProps) {
  ...
  async function share() {
    ...
    if (!blob) return;
    const file = new File([blob], `akari-${loaderData.user.username}.png`, { type: "image/png" });
      ...
      <main id="main-content" className="share-card-main">
        <header className="share-card-heading">
          <div>
            ...
            <p>Choose an AKARI style, protect what stays private, then download or share.</p>
          </div>
          <Link className="quiet-link" to={`/profiles/${loaderData.user.username}`}>View public profile</Link>
        </header>

        {loaderData.saved && <p className="success-banner">Card preferences saved.</p>}
        ...
        <div className="share-card-layout">
          <section className="share-card-stage" aria-label="Card preview">
            ...
            >
              <div className="share-card-brand"><strong>AKARI</strong><span>HOUSE</span></div>
              <span className="share-card-flower" aria-hidden="true">✦</span>
              <div className="share-card-identity">
                <span>{settings.design === "passport" ? "Member passport" : "Member signature"}</span>
                <h2>{loaderData.profile.displayName}</h2>
                <p>@{loaderData.user.username}</p>
                <strong>{title}</strong>
              </div>
              <div className="share-card-metrics">
                <div><strong>{loaderData.opportunityStats.created}</strong><span>Created</span></div>
                <div><strong>{loaderData.opportunityStats.received}</strong><span>Received</span></div>
                <div>
                  <strong>{loaderData.percentile.topPercent ? `Top ${loaderData.percentile.topPercent}%` : "Building"}</strong>
                  <span>{loaderData.percentile.confidence === "verified" ? "Verified percentile" : "AKARI percentile"}</span>
                </div>
              </div>
              <footer>
                <span>{location}</span>
                {languages && <span>{languages}</span>}
                <span>{loaderData.socials.map((social) => social.platform === "x" ? "X" : social.platform[0].toUpperCase()).join(" · ")}</span>
              </footer>
            </article>
          ...
          <Form method="post" className="share-card-controls">
            <fieldset>
              ...
              <label><input type="radio" name="design" value="signature" checked={settings.design === "signature"} onChange={() => setSettings({ ...settings, design: "signature" })} /> Signature</label>
              <label><input type="radio" name="design" value="passport" checked={settings.design === "passport"} onChange={() => setSettings({ ...settings, design: "passport" })} /> Passport</label>
            </fieldset>
            <fieldset>
              <legend>Format</legend>
              <label><input type="radio" name="orientation" value="landscape" checked={settings.orientation === "landscape"} onChange={() => setSettings({ ...settings, orientation: "landscape" })} /> Landscape</label>
              <label><input type="radio" name="orientation" value="portrait" checked={settings.orientation === "portrait"} onChange={() => setSettings({ ...settings, orientation: "portrait" })} /> Portrait</label>
            </fieldset>
            <label>AKARI palette
              <select name="palette" value={settings.palette} onChange={(event) => setSettings({ ...settings, palette: event.target.value as keyof typeof palettes })}>
                {Object.entries(palettes).map(([value, item]) => <option key={value} value={value}>{item.label}</option>)}
              </select>
            </label>
            <label>Country code
              <input name="countryCode" maxLength={2} placeholder="DE" value={settings.countryCode} onChange={(event) => setSettings({ ...settings, countryCode: event.target.value.toUpperCase() })} />
            </label>
            <label className="share-card-check"><input type="checkbox" name="showLocation" checked={Boolean(settings.showLocation)} onChange={(event) => setSettings({ ...settings, showLocation: event.target.checked ? 1 : 0 })} /> Show country and flag</label>
            <label>Languages
              <input name="languages" maxLength={160} placeholder="English, German" value={languages} onChange={(event) => setSettings({ ...settings, languagesJson: JSON.stringify(event.target.value.split(",").map((item) => item.trim()).filter(Boolean)) })} />
            </label>
            <button className="button button-secondary" disabled={navigation.state !== "idle"}>{navigation.state === "idle" ? "Save preferences" : "Saving…"}</button>
            <div className="share-card-actions">
              <button type="button" className="button button-primary" onClick={download}>Download PNG</button>
              <button type="button" className="button button-secondary" onClick={share}>Share card</button>
            </div>
            <small>Sharing always uses your public profile link. Hidden location never appears on the card.</small>
          </Form>
        </div>
      </main>
    </div>
