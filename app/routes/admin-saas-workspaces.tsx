  ...
  return JSON.stringify(
    Object.fromEntries(
      workspaceModules.map((module) => [
        module,
        formText(form.get(`module_${module}`)) === "1",
      ]),
    ),
  );
}
...
export const meta: Route.MetaFunction = () => [
  { title: "SaaS Workspaces | AKARI House" },
  {
    name: "description",
    content:
      "Workspace provisioning, plans, roles, modules and subscription administration.",
  },
];
...
export async function loader({ request, context }: Route.LoaderArgs) {
  ...
  const selectedWorkspaceId = formText(
  ...
  const [workspaces, plans, users, projects] = await Promise.all([
    ...
    db
      .prepare(
        "SELECT id, title, slug FROM projects WHERE status <> 'declined' ORDER BY title COLLATE NOCASE LIMIT 500",
      )
      .all<ProjectOption>(),
  ]);
  ...
  let entitlements: Awaited<
    ReturnType<typeof loadWorkspaceEntitlements>
  > | null = null;
  if (selected) {
    const [memberResult, inviteResult, linkResult, effective] =
      await Promise.all([
        db
          .prepare(
...
export async function action({ request, context }: Route.ActionArgs) {
  ...
  if (intent === "create-plan") {
    ...
    try {
      ...
      if (String(error).includes("UNIQUE"))
        return { error: "Plan code already exists." };
      throw error;
    }
  ...
  if (intent === "create-workspace") {
    ...
    const primaryProjectId =
      formText(form.get("primaryProjectId")).trim() || null;
    ...
    if (billingEmail && !validateEmail(billingEmail))
      return { error: "Enter a valid billing email." };
    const [owner, plan] = await Promise.all([
      db
        ...
        .first(),
      db
        .prepare("SELECT 1 FROM saas_plans WHERE id = ? AND status = 'active'")
        .bind(planId)
        .first(),
    ]);
    ...
    const statements = [
      ...
      db
        ...
        .bind(
          ...
          admin.id,
          workspaceId,
          JSON.stringify({ slug, ownerUserId, planId, primaryProjectId }),
        ),
    ];
    if (primaryProjectId)
      statements.push(
        db
          .prepare(
            "INSERT INTO saas_workspace_project_links (workspace_id, project_id, linked_by) VALUES (?, ?, ?)",
          )
          .bind(workspaceId, primaryProjectId, admin.id),
      );
    try {
    ...
    return {
      saved: true,
      message:
        "Workspace provisioned with an isolated member roster and subscription.",
    };
  }
  ...
  if (intent === "update-workspace") {
    ...
    await db.batch([
      db
        .prepare(
          "UPDATE saas_workspaces SET status = ?, billing_email = ?, brand_name = ?, primary_project_id = ?, updated_by = ?, updated_at = datetime('now') WHERE id = ?",
        )
        .bind(
          status,
          billingEmail,
      ...
      db
        .prepare(
          "INSERT INTO audit_logs (id, actor_user_id, action, subject_type, subject_id, metadata_json) VALUES (?, ?, 'saas_workspace.updated', 'saas_workspace', ?, ?)",
        )
        .bind(
          crypto.randomUUID(),
          admin.id,
          workspaceId,
          JSON.stringify({ status, primaryProjectId }),
        ),
    ]);
  ...
  if (intent === "update-subscription") {
    ...
    const storageOverrideRaw = formText(
      form.get("storageLimitMbOverride"),
    ).trim();
    const seatOverride = seatOverrideRaw
      ? integer(form.get("seatLimitOverride"))
      : null;
    const storageOverride = storageOverrideRaw
      ? integer(form.get("storageLimitMbOverride"))
      : null;
    if (
      !planId ||
      ...
      periodEnd === undefined ||
      (seatOverrideRaw && !seatOverride) ||
      (storageOverrideRaw && !storageOverride)
    )
      return { error: "Check subscription status, dates and overrides." };
    const plan = await db
      .prepare("SELECT 1 FROM saas_plans WHERE id = ?")
      .bind(planId)
      .first();
  ...
  if (intent === "set-entitlement") {
    ...
    if (!isWorkspaceModule(moduleKey))
      return { error: "Unknown workspace module." };
    await db
      .prepare(
  ...
  if (intent === "add-member") {
    ...
    if (
      ...
      Number(count?.count ?? 0) >= effective.seatLimit
    )
      return {
        error:
          "Seat limit reached. Increase the subscription seat limit before adding this member.",
      };
    await db
      .prepare(
  ...
  if (intent === "update-member") {
    ...
    await db
      .prepare(
        "UPDATE saas_workspace_members SET role = ?, status = ?, updated_at = datetime('now') WHERE workspace_id = ? AND user_id = ?",
      )
  ...
  if (intent === "update-invite") {
    ...
    await db
      .prepare(
        "UPDATE saas_workspace_invitations SET status = ?, updated_at = datetime('now') WHERE id = ? AND workspace_id = ?",
      )
      .bind(status, inviteId, workspaceId)
      .run();
  ...
  if (intent === "link-project") {
    ...
    await db
      .prepare(
        "INSERT OR IGNORE INTO saas_workspace_project_links (workspace_id, project_id, linked_by) VALUES (?, ?, ?)",
      )
      .bind(workspaceId, projectId, admin.id)
      .run();
  ...
  if (intent === "unlink-project") {
    ...
    await db
      .prepare(
        "DELETE FROM saas_workspace_project_links WHERE workspace_id = ? AND project_id = ?",
      )
...
export default function AdminSaasWorkspaces({
  loaderData,
  actionData,
    ...
    <div className="dashboard-shell">
      ...
      <main id="main-content" className="directory-main">
        ...
        <header className="directory-heading">
          <div>
            <span className="eyebrow">SaaS administration</span>
            <h1>Provision and govern AKARI workspaces.</h1>
            <p>
              Tenant roster, plan, subscription, module and Project-link control
              without exposing House-wide internal data.
            </p>
          </div>
        </header>
        {actionData?.error && (
        ...
        <section className="status-card">
          <div className="section-heading">
            <div>
              <span className="chapter">Provision</span>
          ...
          <Form method="post" className="profile-form">
            ...
            <div className="form-grid two-column-grid">
              ...
              <label>
                Owner
                <select name="ownerUserId" defaultValue="" required>
              ...
              <label>
                Plan
                <select name="planId" defaultValue="plan-house-internal">
                  {loaderData.plans
        ...
        <section className="status-card">
          <div className="section-heading">
            <div>
              <span className="chapter">Plans</span>
              <h2>Create commercial plan</h2>
            </div>
            <p>Pricing is admin-defined. No payment processor is implied.</p>
          </div>
          <Form method="post" className="profile-form">
            ...
            <div className="chip-list">
              {workspaceModules.map((module) => (
                <label className="checkbox-row" key={module}>
                  <input type="checkbox" name={`module_${module}`} value="1" />{" "}
              ...
              <tbody>
                {loaderData.plans.map((p) => (
                  <tr key={p.id}>
                    ...
                    <td>{moneyLabel(p.monthlyPriceCents, p.currency)}/mo</td>
                    <td>{p.seatLimit}</td>
        ...
        <section className="status-card">
          <div className="section-heading">
            <div>
              <span className="chapter">Workspace register</span>
              <h2>{loaderData.workspaces.length} workspaces</h2>
            </div>
          </div>
          <div className="table-scroll">
            <table className="admin-table">
              ...
              <tbody>
                {loaderData.workspaces.map((w) => (
                  <tr key={w.id}>
                    ...
                    <td>{w.ownerName}</td>
                    <td>{w.projectTitle ?? "None"}</td>
                    <td>
                      {w.planName ?? "Unconfigured"}
                      <br />
                      <small>{w.subscriptionStatus ?? ""}</small>
                    </td>
                    <td>{w.memberCount}</td>
                    <td>{w.status}</td>
                    <td>
                      <button
                        type="button"
                        onClick={() => setSearchParams({ workspace: w.id })}
                      >
                        Manage
                      </button>{" "}
                      <Link to={`/workspaces/${w.slug}`}>Workspace</Link>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        {selected && (
          <>
            <section className="status-card">
              <div className="section-heading">
                <div>
                  <span className="chapter">Selected workspace</span>
                  <h2>{selected.name}</h2>
                </div>
                <Link to={`/workspaces/${selected.slug}`}>
                  Open member shell
                </Link>
              </div>
              <Form method="post" className="profile-form">
                ...
                <div className="form-grid two-column-grid">
                  ...
                  <label>
                    ...
                    <input
                      name="brandName"
                      maxLength={120}
                      defaultValue={selected.brandName}
                    />
                  </label>
                  <label>
                    Primary Project
                    <select
                      ...
                      defaultValue={selected.primaryProjectId ?? ""}
                    >
                      <option value="">None</option>
                      {loaderData.projects.map((p) => (
                        <option key={p.id} value={p.id}>
            ...
            <section className="status-card">
              <div className="section-heading">
                <div>
                  <span className="chapter">Subscription</span>
                  <h2>{loaderData.entitlements?.planName}</h2>
                </div>
                <p>
                  {loaderData.members.length}/
                  {loaderData.entitlements?.seatLimit ?? 0} active roster
                  entries · storage limit is configured, current storage is not
                  fabricated until R2 objects are workspace-attributable.
                </p>
              </div>
              <Form method="post" className="profile-form">
                <input
                ...
                <div className="form-grid two-column-grid">
                  <label>
                    ...
                    <select
                      name="planId"
                      defaultValue={selected.planId ?? "plan-house-internal"}
                    >
                      {loaderData.plans.map((p) => (
                        <option key={p.id} value={p.id}>
                          {p.name}
                  ...
                  <label>
                    ...
                    <select
                      name="status"
                      defaultValue={selected.subscriptionStatus ?? "trialing"}
                    >
                  ...
                  <label>
                    ...
                    <input
                      name="trialEndsAt"
                      type="date"
                      defaultValue={selected.trialEndsAt?.slice(0, 10) ?? ""}
                    />
                  </label>
                  <label>
                    Period start
                  ...
                  <label>
                    ...
                    <input
                      ...
                      type="date"
                      defaultValue={
                        selected.currentPeriodEnd?.slice(0, 10) ?? ""
                      }
                    />
                  </label>
                  <label>
                  ...
                  <label>
                    Seat override
                    <input name="seatLimitOverride" type="number" min={1} />
                  </label>
                  <label>
                    Storage MB override
                    <input
                      name="storageLimitMbOverride"
                      type="number"
                      min={1}
                    />
                  </label>
                </div>
                <button disabled={pending}>Save subscription</button>
              </Form>
              <div className="chip-list">
                {workspaceModules.map((module) => (
                  <Form method="post" className="inline-form" key={module}>
                    <input
                      type="hidden"
                      name="intent"
                      value="set-entitlement"
                    />
                    <input
                      type="hidden"
                      name="workspaceId"
                      value={selected.id}
                    />
                    <input type="hidden" name="moduleKey" value={module} />
                    <span>
                      {workspaceModuleLabels[module]}:{" "}
                      <strong>
                        {loaderData.entitlements?.modules[module]
                          ? "On"
                          : "Off"}
                      </strong>
                    </span>
                    <select
                      name="enabled"
                      defaultValue={
                        loaderData.entitlements?.modules[module] ? "1" : "0"
                      }
                    >
                      <option value="1">On</option>
                      <option value="0">Off</option>
                    </select>
                    <button disabled={pending}>Override</button>
                  </Form>
                ))}
              </div>
            </section>
            ...
            <section className="status-card">
              ...
              {loaderData.invites.map((invite) => (
                <Form method="post" className="inline-form" key={invite.id}>
                  <input type="hidden" name="intent" value="update-invite" />
                  <input type="hidden" name="workspaceId" value={selected.id} />
                  <input type="hidden" name="inviteId" value={invite.id} />
                  <span>
                    {invite.email} · {invite.role}
                  </span>
                  <select name="status" defaultValue={invite.status}>
            ...
            <section className="status-card">
              <div className="section-heading">
                <div>
                  <span className="chapter">Project boundary</span>
                  <h2>Linked Projects</h2>
                </div>
              </div>
              <Form method="post" className="inline-form">
                <input type="hidden" name="intent" value="link-project" />
                <input type="hidden" name="workspaceId" value={selected.id} />
                ...
                <button disabled={pending}>Link Project</button>
              </Form>
              {loaderData.linkedProjects.map((p) => (
                <Form method="post" className="inline-form" key={p.id}>
                  <input type="hidden" name="intent" value="unlink-project" />
                  <input type="hidden" name="workspaceId" value={selected.id} />
                  <input type="hidden" name="projectId" value={p.id} />
                  <span>{p.title}</span>
                  <button disabled={pending}>Unlink</button>
                </Form>
              ))}
            </section>
          </>
