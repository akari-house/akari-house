    ...
    <fieldset className="role-grid">
      ...
      {(Object.keys(roleCopy) as Role[]).map((role) => (
        <label className="role-card" key={role}>
          <input type="checkbox" name="roles" value={role} defaultChecked={selected.includes(role)} />
          <span className="role-glyph" aria-hidden="true">{role === "founder" ? "創" : role === "creator" ? "彩" : "見"}</span>
