...
async function loadRoles(db: D1Database, userId: string): Promise<Role[]> {
  const result = await db.prepare("SELECT role FROM user_roles WHERE user_id = ? ORDER BY role").bind(userId).all<{ role: Role }>();
  return result.results.map((row) => row.role);
}

export async function getOptionalUser(request: Request, db: D1Database): Promise<SessionUser | null> {
  ...
  ).bind(tokenHash).first<{ id: string; email: string; username: string; displayName: string }>();
  if (!row) return null;
