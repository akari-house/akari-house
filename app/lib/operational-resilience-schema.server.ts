...
export async function ensureOperationalResilienceSchema(db: D1Database) {
  if (schemaReady) return;
  await db.batch([
    db.prepare(`CREATE TABLE IF NOT EXISTS operational_runs (
      id TEXT PRIMARY KEY,
    ...
    db.prepare(
      `CREATE INDEX IF NOT EXISTS idx_managed_r2_source ON managed_r2_objects(source_type, source_id)`,
    ),
    db.prepare(`CREATE TABLE IF NOT EXISTS r2_inventory_findings (
    ...
    db.prepare(
      `CREATE INDEX IF NOT EXISTS idx_r2_inventory_findings_open ON r2_inventory_findings(finding_type, resolved_at, last_seen_at DESC)`,
    ),
